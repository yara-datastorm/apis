        ...
        stage('Analyze') {
            steps {
                script {
                    ...
                    sh 'docker run --rm -v "//var/run/docker.sock:/var/run/docker.sock" --mount type=bind,source="\$(pwd)"/vuln-scan,target=/home aquasec/trivy:0.18.3 image --skip-update true --timeout 10m0s --no-progress true --format template --template @contrib/html.tpl -o ./home/trivy-ci-report-os-library$BUILD_NUMBER.html --ignore-unfixed --exit-code 0 --vuln-type os,library  --severity CRITICAL,HIGH $IMAGE_TAG_NAME:$BUILD_NUMBER'
